import { AlertTriangle, ArrowLeft, Building2, Check, CheckCircle2, FilePlus2, Link, LoaderCircle, Moon, Pencil, Plus, RefreshCw, Trash2, Upload, X } from "lucide-react";
import { useMemo, useRef, useState, type FormEvent } from "react";
import type { RequirementStatus, TransactionCustomField, TransactionDocument, TransactionFile, TransactionFinancial, TransactionImportantDate, TransactionLinkageProposal, TransactionOwnerOption, TransactionParty, TransactionPartyRole, TransactionRequirement, TransactionTypeOption } from "./infrastructure/supabase/transactions.js";
        ...
        {creating ? <form className="transaction-create" onSubmit={(event) => void create(event)}><div><h2>Create Transaction File</h2><p>Add the requirements needed before this transaction can be sent for final review.</p></div>
          ...
          <section className="documents-section">
            ...
            <div className="document-list">
              ...
              {selected.pendingDocumentUploads?.map((upload) => <article className="document-row pending" key={upload.id}><div className={`document-status ${upload.status === "FAILED" ? "status-rejected" : ""}`}>{upload.status === "FAILED" ? <AlertTriangle size={16} /> : <LoaderCircle className="processing-spinner" size={16} />}</div><div><strong>{upload.documentName}</strong><span>{upload.status === "FAILED" ? "Upload could not be completed" : upload.processingStatus === "RUNNING" ? "Safety check in progress" : "Safety check pending"}</span>{upload.failureReason ? <small>{upload.failureReason}</small> : <small>{upload.processingStatus === "QUEUED" ? "The file is uploaded and waiting for its safety check." : "The file is uploaded and being checked."}</small>}</div><span className={`document-state ${upload.status === "FAILED" ? "state-rejected" : ""}`}>{upload.status === "FAILED" ? "Action needed" : "Processing"}</span>{props.onCancelDocumentUpload && upload.status === "WAITING_FOR_SCAN" ? <div className="document-actions"><button className="secondary-button" type="button" disabled={pending} onClick={() => void run(() => props.onCancelDocumentUpload!(selected, upload.ingestionEventId), "Document upload cancelled.")}>Cancel upload</button></div> : null}</article>)}
              {selected.documents?.map((document) => <article className="document-row" key={document.id}><div className={`document-status status-${document.status.toLowerCase()}`}><FilePlus2 size={16} /></div><div><strong>{document.name}</strong><span>{document.fileName} · Version {document.version}</span>{document.decisionReason ? <small>{document.decisionReason}</small> : null}</div><span className={`document-state state-${document.status.toLowerCase()}`}>{humanize(document.status)}</span>{document.status === "RECEIVED" && props.onReviewDocument ? <div className="document-actions"><button className="secondary-button" type="button" disabled={pending} onClick={() => setDocumentDecision({ document, decision: "REJECTED" })}><X size={14} /> Reject</button><button className="approve-button" type="button" disabled={pending} onClick={() => setDocumentDecision({ document, decision: "VERIFIED" })}><Check size={14} /> Verify</button></div> : null}</article>)}
          ...
          <section className="requirements-section"><header><div><h3>Requirements</h3><p>{selected.requirements.length === 0 ? "Add at least one requirement before final review." : blockers.length === 0 ? "All requirements are complete." : `${blockers.length} ${blockers.length === 1 ? "item needs" : "items need"} attention.`}</p></div>{props.onAddRequirement && !["DORMANT", "ARCHIVED"].includes(selected.lifecycle) ? <button className="secondary-button" type="button" onClick={() => setAddingRequirement((open) => !open)}><Plus size={14} /> Add requirement</button> : null}</header>
            ...
            <div className="requirement-list">{selected.requirements.map((requirement) => {
              const uploadInProgress = localUpload?.requirementKey === requirement.key || selected.pendingDocumentUploads?.some((upload) => upload.requirementKey === requirement.key && upload.status === "WAITING_FOR_SCAN");
              return <div className="requirement-row" key={`${requirement.kind}:${requirement.key}`}>
                ...
                {requirement.kind === "ARTIFACT" ? <div className="requirement-upload-control"><button className="secondary-button document-upload-button" type="button" disabled={pending || uploadInProgress || !props.onUploadDocument || selected.businessStage === "CLOSED" || selected.businessStage === "CANCELLED" || selected.lifecycle === "DORMANT"} onClick={() => { setUploadRequirement(requirement); window.setTimeout(() => documentInput.current?.click(), 0); }}>{uploadInProgress ? <LoaderCircle className="processing-spinner" size={14} /> : <Upload size={14} />} {uploadInProgress ? "Processing..." : pending && uploadRequirement === requirement ? "Uploading..." : requirement.status === "PRESENT" ? "Replace file" : "Upload document"}</button><small>{uploadInProgress ? "File uploaded · Safety check pending" : "PDF, JPG or PNG · Maximum 5 MB"}</small></div> : <span className="requirement-location">See Required information</span>}
          ...
          {selected.businessStage === "UNDER_REVIEW" ? <section className="review-submitted" aria-label="Submitted for review"><CheckCircle2 size={19} /><div><strong>Review submitted</strong><span>{selected.requirements.filter((item) => item.stageGate === "BEFORE_REVIEW").length > 0 ? "The completed Before Review items shown above are now awaiting review." : "No review-gated documents were required for this Transaction File."}</span></div></section> : null}
